# -*- coding: utf-8 -*-
"""
Created on Fri Dec  8 18:31:38 2023

original name: CPCf_Wind_Dir_and_Speed_FiLt_NEW

@author: sirna

Content:
    downloading hdf5 files
    applying purge and quality control to cpc
    filtering based on wind direction 110-225 and wind speed < 5 m/s
...
master_frame['concentration_scrubbed']=cpc_set_purge['concentration_scrubbed']
master_frame['wind_direction_purged']= wind_set_purge['wind_direction_purged']
master_frame['wind_speed_purged'] = wind_set_purge['wind_speed_purged']
master_frame['stack_purge_state'] =cpc_set_purge['stack_purge_state']
master_frame['concentration']= cpc_set_purge['concentration']
...
'''
The for loop has the conditions used for filtering
A filtered list is created that saves values from the concentration scrubbed trace
that are not flagged by the conditions
Values flagged by the conditions are assigned a NAN value

Conditions of Flag: within 110-225 and < 5m/s are removed
'''


total_len=len(master_frame['concentration_scrubbed'])

import math

concentration_filtered=[None]*total_len

for i in range(len(concentration_filtered)):
    if ((master_frame['wind_speed_purged'][i]<5) and (master_frame['wind_direction_purged'][i]>110) and (master_frame['wind_direction_purged'][i]< 225)):
        concentration_filtered[i]=math.nan
    else: concentration_filtered[i]=master_frame['concentration_scrubbed'][i]
     
master_frame['filtered_concentration'] = concentration_filtered

#%% Statistical Anaysis 

'''
Statistical analysis for change in length of data frame
which indicates how much data was dropped and changes in mean and median 
...
axs[2].set_title('(b) Filtered Concentration', fontname="Times New Roman" , fontsize = 25)
...
'''
Two data frames are made, one for filtered trace and one for unfiltered trace
both are passed through the function to create the degree average for polar
plotting
'''
...
'''
Prints for the meteorological filter the statistical data
'''


print('Meteorological Filtering: removing 110-225 deg. and < 5m/s')
print(' ')
...
print('Percent Change in Median: ', delta_median_percent)
